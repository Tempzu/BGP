...
namespace BGP_Router.BGP
{
    public class InitializingListenersSpeakers
    {
        ...
        public void StartListener() // user can configure the autonomous systems
        {
            ...
            try
            {
                Variables.as1Last = int.Parse(temp) - 1; //-1 since routers start from 0 
                if (Variables.as1Last < 0)
                {
                    Console.WriteLine("The character you typed wasn't in the given bounds, so we set 1 router in AS1.");
                    Variables.as1Last = 0;
                }
                else if (Variables.as1Last > 7)
                {
            ...
            if (Variables.as1Last < 7)
            {
                Variables.as2First = Variables.as1Last + 1;
                if (Variables.as1Last > 0)
                {
                    Console.WriteLine("How many routers do you want in AS2? (1-" + (8 - Variables.as1Last) + "). Rest will be placed in AS3.");
                }
                else
                {
                ...
                try
                {
                    Variables.as2Last = Variables.as1Last + int.Parse(temp);
                    if (Variables.as2Last <= Variables.as1Last)
                    {
                        Console.WriteLine("The character you typed wasn't in the given bounds, so we set 1 router in AS2.");
                        Variables.as2Last = Variables.as1Last + 1;
                    }
                    else if (Variables.as2Last > 8)
                    {
                ...
                Variables.as3First = Variables.as2Last + 1;
            }
            else
            {
                Console.WriteLine("One router will be now placed in both remaining AS.");
                Variables.as2First = 8;
                Variables.as2Last = Variables.as2First;
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                else if (i > Variables.as1Last && i < Variables.as3First)
                {
                    AS = Variables.AS2;
                    Variables.ListenerConnectionAndAS.TryAdd((ushort)i, AS);
                ...
                else if (i > Variables.as2First && i < 10)
                {
                    AS = Variables.AS3;
                    Variables.ListenerConnectionAndAS.TryAdd((ushort)i, AS);
        ...
        public void StartSpeaker() //Create the speaker sockets for the routers
        {
            for (int k = 0; k < 10; k++)
            {
                ...
                else if (k > Variables.as1Last && k < Variables.as3First)
                {
                    AS = Variables.AS2;
                    if (k == Variables.as2First)
                    {
                    ...
                    else if (Variables.as2First < k && k < Variables.as2Last)
                    {
                        BGPSpeaker[m].BindSpeaker(Variables.As2_IP_Prefix + k, Variables.SpeakerPortNumber, m);
                        Variables.SpeakerAS.TryAdd(Variables.As2_IP_Prefix + k, AS);
                    }
                ...
                else if (k > Variables.as2Last && k < 9)
                {
                    AS = Variables.AS3;
        ...
        public void SpeakerConnection_Init() //Initialize the speakers
        {
            for (int k = 0; k < 10; k++)
            {
                ...
                if (k < Variables.as2First)
                {
                    ...
                    if (k == Variables.as1Last) //creating connection which connects to the first router of AS2
                    {
                        ...
                        if (1 < Variables.as1Last) //more connections are created to AS1 depending on the size of AS1
                        {
                            n++;
                            Variables.SpeakerConnectionAndAS.TryAdd((ushort)n, Variables.AS1);
                    ...
                    else if (0 < Variables.as1Last) //creating normal connections to AS1 (n -> n+1 )
                    {
                ...
                else if (k > Variables.as1Last && k < Variables.as3First) //creating connections to AS2
                {

                    Variables.SpeakerConnectionAndAS.TryAdd((ushort)n, Variables.AS2);
                    Variables.ListenerConnectionCount = n;
                    if (k == Variables.as2First && (Variables.as2Last - Variables.as2First) > 0) //more connections are created to AS2 depending on the size of AS2
                    {
                        BGPSpeaker[n].Connect(Variables.As2_IP_Prefix + (k + 1), Variables.ListenerPortNumber, k, k + 1);
                        Variables.ConnectionAndListener.TryAdd(n, Variables.As2_IP_Prefix + (k + 1));
                        Variables.ConnectionAndSpeaker.TryAdd(n, Variables.As2_IP_Prefix + k);
                        SendOpenMessageToListener(n);
                        if (k == Variables.as2First && (Variables.as2Last - Variables.as2First) > 1)
                        {
                            n++;
                            Variables.SpeakerConnectionAndAS.TryAdd((ushort)n, Variables.AS2);
                    ...
                    else if (k == Variables.as2Last) //creating connection which connects to the first router of AS3
                    {
                        ...
                        if ((Variables.as2Last - Variables.as2First) > 1) //more connections are created to AS2 depending on the size of AS2
                        {
                            n++;
                            Variables.SpeakerConnectionAndAS.TryAdd((ushort)n, Variables.AS2);
                            BGPSpeaker[n].Connect(Variables.As2_IP_Prefix + (k - 2), Variables.ListenerPortNumber, k, k - 2);
                            ...
                            Variables.ConnectionAndSpeaker.TryAdd(n, Variables.As2_IP_Prefix + k);
                            SendOpenMessageToListener(n);
                            if ((Variables.as2Last - Variables.as2First) > 2)
                            {
                                n++;
                                Variables.SpeakerConnectionAndAS.TryAdd((ushort)n, Variables.AS2);
                    ...
                    else if (Variables.as2First < k && k < Variables.as2Last) //creating normal connections to AS2 (n -> n+1 )
                    {
                        BGPSpeaker[n].Connect(Variables.As2_IP_Prefix + (k + 1), Variables.ListenerPortNumber, k, k + 1);
                ...
                else if (k == 9) //creating connection to the first router of AS1
                {
                    ...
                    Variables.ConnectionAndSpeaker.TryAdd(n, Variables.As3_IP_Prefix + k);
                    SendOpenMessageToListener(n);
                    if ((k - (Variables.as2Last + 1)) > 1)//more connections are created to AS3 depending on the size of AS3
                    {
                        n++;
                        Variables.SpeakerConnectionAndAS.TryAdd((ushort)n, Variables.AS3);
